...
// Reusable display function to keep the UI clean
void refreshUI() {
    std::lock_guard<std::mutex> lock(console_mtx);
    // \033[H moves cursor to top-left, \033[J clears from there down
    std::cout << "\033[H\033[J"; 
    std::cout << "========= SYSTEM MONITOR (MT) =========\n";
...
void calcCpuUsage() {
    auto read_cpu = []() {
        ...
        unsigned long u, n, s, i, iw, irq, sirq, st;
        std::string label;
        file >> label >> u >> n >> s >> i >> iw >> irq >> sirq >> st;
        return std::make_pair(u + n + s + i + iw + irq + sirq + st, i + iw);
    };
